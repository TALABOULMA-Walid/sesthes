...
\node[below of = fmu1,yshift=-1cm,xshift=1.5cm] {\textbf{Step 1}};
\end{scope}
...
\node[below of = fmu1,yshift=-1cm,xshift=1.5cm] {\textbf{Step 2}};
\end{scope}
...
\node at(0,-5) {\footnotesize{Each FMU: 2 inputs + 2 outputs + 1 state}};
\node at(0,-5.5) {\small{Total = 10 operations}};
...
\node at(0,-5) (exp) {Input/Output to state arcs};
\draw [-{Latex[]},draw=green,line width=.8pt] ([xshift=-.5cm]exp.west) -- (exp);

\node at(0,-4.5) {\textbf{Step 4}};
\end{scope}

\begin{scope}[yshift=-16cm,xshift=4cm]
% FMU1
\node[rectangle,draw,minimum height = 2.4cm,minimum width = 2cm] (fmu1) {};
...
%Inter-FMU
\coordinate [right of =fmu2, yshift=2cm,xshift=.5cm] (c1);
\coordinate [left of =fmu1, yshift=2cm,xshift=-.5cm] (c2);

\coordinate [right of =fmu2, yshift=-2cm,xshift=.5cm] (c3);
\coordinate [left of =fmu1, yshift=-2cm,xshift=-.75cm] (c4);
...
\node[below of = fmu1,yshift=-1.5cm,xshift=1.5cm] {\textbf{Resulting co-simulation}};
\end{scope}
\end{tikzpicture}
