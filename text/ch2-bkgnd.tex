...
Parallel computers are of many types, some of which are adapted only to specific kinds of applications. Parallel computers can be classified according to different criteria. The well-known Flynn's Taxonomy \cite{flynn:1972} classifies computers according to instruction and data streams into the following categories:

%the physical organization of processors and memory into the following types:

\subsubsection{Single-Instruction Stream--Single-Data Stream (SISD)}
...
\subsubsection{Single-Instruction Stream--Multiple-Data Streams (SIMD)}
...
\subsubsection{Multiple Instruction-Streams--Single Data Stream (MISD)}
...
\subsubsection{Multiple Instruction-Streams--Multiple Data Streams (MIMD)}

Different instructions are executed on different data in parallel. Examples of MIMD computers include multiprocessors and multi-core architectures.
