...
We propose in this section a heuristic for the acyclic orientation of the mixed graph $G(V,A,D)$. First, we introduce some notations and concepts that are used in the proposed heuristic.
...
In view of the above, the acyclic orientation of $G(V,A,D)$, $\alpha: \{[o_i,o_j] \in D\} \rightarrow \{(o_i),(o_j)\}$, is obtained by applying a sequence of mappings $\alpha_1, \alpha_2, \ldots, \alpha_{|D|}$, such that every mapping $\alpha_l$ takes into the consideration only one subset of operations $V_k, k \in \{0, 1, \ldots, d\}$ and assigns a direction to one edge in $D_k$. The number of mappings needed to perform the acyclic orientation of $G(V,A,D)$ is equal to the number of edges $|D|$.  
