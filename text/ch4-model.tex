...
\item If $succ(o_i^p) = \emptyset$, stop. Otherwise, propagate the constraint assigned to every operation $o_i^p$ in the previous step towards all its successors $o_{i'} \in succ(o_i^p)$.
...
\item If $pred(o_j^q) = \emptyset$, stop. Otherwise, propagate the constraint assigned to every operation $o_j^q$ in the previous step towards all its predecessors $o_{j'} \in pred(o_j^q)$.
...
\begin{equation}
\forall o_i^p \in V, o_i^{p'}: p' = p - \frac{HP}{H(o_i)},\ R(o_i^{p'}) = R(o_i^p) - HP
\label{eq:release}
\end{equation}
