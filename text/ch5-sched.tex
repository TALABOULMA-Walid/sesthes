...
\begin{algorithm}[!htp]		
	  ...
	  \Output{Schedule of operations $o_i \in V$ on cores $p_k \in P$\;}  
 		...
 		\While{$O \neq \emptyset$}
		{
  		 ...
  		 $L_{best} \leftarrow L_{best} + C(o_{i'})$\;
...
\begin{tabular}{l  l}
  \rule{0pt}{5ex}	
	\textit{Input} & Operation graph $G(V,A)$\\
	\rule{0pt}{5ex}									  
	
  \textit{Output} & Offline Schedule of operations on a multi-core processor\\
	\rule{0pt}{5ex}									  
	...
	\textit{Subject to} & Precedence constraints: $\forall (o_i,o_j) \in A, S(o_j) \geq E(o_i)$\\
...
\begin{algorithm}[!htp]		
 		...
 		\While{$O \neq \emptyset$}
		{
  		 ...
  		 Find $o_{i'}$ with the smalelst priority $\rho$ in $O$\; 
  		 Schedule $o_{i'}$ on its core $BestCore(o_{i'})$\;
